...
model <- function(t, x, parms) {
  with(as.list(c(parms, x)), {
    ...
    tau_c <- sapply(1:numstages, function(k) parms[paste0("tau_c", k)])
    ...
    Cp <- sapply(1:numgroups, function(l) x[paste0("Cp", l)])
    Icp <- sapply(1:numgroups, function(l) x[paste0("Icp", l)])
    Ic <- matrix(sapply(1:(numgroups * numstages), function(i) x[paste0("Ic", i)]), nrow = numgroups, ncol = numstages)
    
    Ng <- rowSums(cbind(S, Sp, rowSums(I), Ip, D, A, C, R, Cp, rowSums(Ic), Icp))
    N <- sum(Ng)
    ...
    for(l in 1:numgroups){
      for (ll in 1:numgroups) {
        J[l] <- J[l] + M[l, ll] * (eps_p * (Ip[ll] + Icp[ll]) / Ng[ll] + eps * (A[ll] + R[ll]) / Ng[ll] + eps_d * D[ll] / Ng[ll])
    ...
    if(t<times[dt*(prep_fit_idxs[1]-1)+1]){
      ...
      kontime[3] <- kon[3] / (1 + exp(-1*(-sum(Sp)+thr_prep)) )
      kontime[4] <- kon[4] / (1 + exp(-1*(-sum(Sp)+thr_prep)) )
      kontime_c[3] <- kon[3] / (1 + exp(-1*(-sum(Cp)+thr_prep)) )
      kontime_c[4] <- kon[4] / (1 + exp(-1*(-sum(Cp)+thr_prep)) )
      dS <- beta * NN * q + kof * Sp - (mu + J + kontime) * S
      dSp <- kontime * S - (mu + Omega * J + kof) * Sp
      dC <- - (mu + J + kontime_c) * C + eff*alphatime * A + kof * Cp
      dCp <- kontime_c * C - (mu + Omega * J + kof) * Cp
      dR <- + (1.-eff)*alphatime * A - (mu + gamma) * R
      dIp <- Omega * J * Sp - (mu + tau_p) * Ip
      dD <- tau_p * (Ip + Icp) - (eta + mu + gamma_diag) * D
      for (k in 1:numstages) {
        dD <- dD + I[,k]*tau[k] + tau_c[k] * Ic[,k]
      }
      dA <- eta * D - (mu + gamma + alphatime) * A + qadj * m_imp * t
      dIc[,1] <- J * C - (mu + rho[1] + tau_c[1]) * Ic[,1]
      dI[,1] <- J * S - (mu + rho[1] + tau[1]) * I[,1] + beta * NN * qadj *(imp * prob_stages[1])
      for (k in 2:numstages) {
        dIc[,k] <- rho[k-1]*Ic[,k-1] - (mu+rho[k]+tau_c[k])*Ic[,k]
        dI[,k] <- rho[k-1]*I[,k-1] - (mu+rho[k]+tau[k])*I[,k] + beta * NN * qadj *(imp * prob_stages[k])
      }
      dIcp <- Omega * J * Cp - (mu + tau_p) * Icp
    }
    res <- c(dS, dSp, dI, dIp, dA, dD, dC, dR, dCp, dIc, dIcp)
    list(res)
  })
}
...
# Define the cure uptakes
alphas <- c(-log(1-0.9), -log(1-0.5), -log(1-0.1), 0)
alphas <- rev(alphas)
n_sa <- length(alphas)
...
incidence_undiag_naive_after_new_time_pool <- array(0,dim=c(n_monitors,n_sa,len_post,nt))
incidence_undiag_nonnaive_after_new_time_pool <- array(0,dim=c(n_monitors,n_sa,len_post,nt))
...
incidence_undiag_naive <- rep(0,nt)
incidence_undiag_nonnaive <- rep(0,nt)
...
Cp_out <- matrix(0,nt,numgroups)
Icp_out <- matrix(0,nt,numgroups)
...
# for (g in 2:2) {
  for(f in alphas){
    ...
    for(i in 1:len_post){
      ...
      Cp_0 <- rep(0,numgroups)
      Icp_0 <- rep(0,numgroups)
      I_0 <- matrix(0, numgroups, numstages)
      for (k in 1:numstages) {
      ...
      xstart <- c(S=S_0, Sp=Sp_0, I=I_0, Ip=Ip_0, A=A_0, D=D_0, C=C_0, R=R_0, Cp=Cp_0, Ic=Ic_0, Icp=Icp_0)
      ...
      for(l in 1:numgroups){
        ...
        Cp_out[,l] <- out[,paste0("Cp",l)]
        Icp_out[,l] <- out[,paste0("Icp",l)]
        for(k in 1:numstages){
      ...
      for (t in 1:nt) {
        ...
        Ng_out[t,] <- S_out[t,] + Sp_out[t,] + Ip_out[t,] + D_out[t,] + A_out[t,] + sumIl + C_out[t,] + R_out[t,] + Cp_out[t,] + sumIcl + Icp_out[t,]
      }
      ...
      for (t in 1:nt) {
        ...
        J <- rep(0,numgroups)
        J_undiag_naive <- rep(0,numgroups)
        J_undiag_nonnaive <- rep(0,numgroups)
        ...
        for(l in 1:numgroups){
          for (ll in 1:numgroups) {
            ...
            J_undiag_naive[l] <- J_undiag_naive[l] +  M[l, ll] * transmissibility_prep * (Ip_out[t,ll]) / Ng_out[t,ll]
            J_undiag_nonnaive[l] <- J_undiag_nonnaive[l] +  M[l, ll] * transmissibility_prep * (Icp_out[t,ll]) / Ng_out[t,ll]
            J[l] <- J[l] + M[l, ll] * (transmissibility_prep * (Ip_out[t,ll] + Icp_out[t,ll]) / Ng_out[t,ll] + parms["eps"] * (A_out[t,ll] + R_out[t,ll]) / Ng_out[t,ll] + transmissibility_diagnosed * D_out[t,ll] / Ng_out[t,ll])
            for (k in 1:numstages) {
              J_undiag_naive[l] <- J_undiag_naive[l] +  M[l, ll] * (infectivity[k] * I_out[t,ll,k]) / Ng_out[t,ll]
              J_undiag_nonnaive[l] <- J_undiag_nonnaive[l] +  M[l, ll] * (hc[k] * Ic_out[t,ll,k]) / Ng_out[t,ll]
          ...
          J_undiag_naive[l] <- J_undiag_naive[l] * (parms["lambda"] * contact_rates[l])
          J_undiag_nonnaive[l] <- J_undiag_nonnaive[l] * (parms["lambda"] * contact_rates[l])
        }
        ...
        incidence_undiag_naive[t] <- sum(J_undiag_naive*S_out[t,]) + effectiveness_prep * sum(J_undiag_naive*Sp_out[t,])
        incidence_undiag_nonnaive[t] <- sum(J_undiag_nonnaive*S_out[t,]) + effectiveness_prep * sum(J_undiag_nonnaive*Sp_out[t,])
        ...
        incidence_cure[t] <- sum(J*C_out[t,]) + effectiveness_prep * sum(J*Cp_out[t,])
        prevalence_cure[t] <-  sum(Ic_out[t,,]) + sum(Icp_out[t,])
      ...
      coverage_time_pool[g,which(alphas==f),i,] <- coverage
      incidence_after_new_time_pool[g,which(alphas==f),i,] <- incidence
      incidence_art_after_new_time_pool[g,which(alphas==f),i,] <- incidence_art
      incidence_diag_after_new_time_pool[g,which(alphas==f),i,] <- incidence_diag
      incidence_undiag_naive_after_new_time_pool[g,which(alphas==f),i,] <- incidence_undiag_naive
      incidence_undiag_nonnaive_after_new_time_pool[g,which(alphas==f),i,] <- incidence_undiag_nonnaive
      incidence_after_reb_time_pool[g,which(alphas==f),i,] <- incidence_cure
      prevalence_after_new_time_pool[g,which(alphas==f),i,] <- prevalence
      prevalence_after_reb_time_pool[g,which(alphas==f),i,] <- prevalence_cure
      newly_cured_time_pool[g,which(alphas==f),i,] <- newly_cured
      art_given_time_pool[g,which(alphas==f),i,] <- art_given
    }
  }
}
...
mean_inc_undiag_naive_new <- array(apply(incidence_undiag_naive_after_new_time_pool, c(1, 2, 4), mean), dim=c(n_monitors, n_sa, nt))
mean_inc_undiag_nonnaive_new <- array(apply(incidence_undiag_nonnaive_after_new_time_pool, c(1, 2, 4), mean), dim=c(n_monitors, n_sa, nt))
...
save.image(paste0("RESULTS/SCENARIOS/SC2_eff", round(parms["eff"]*100,0), "_hc", hcfile,"_curestart",y_cure,".RData"))
