...
model <- function(t, x, parms) {
  with(as.list(c(parms, x)), {
    ...
    if(t<times[dt*(prep_fit_idxs[1]-1)+1]){
      ...
      dS <- beta * NN * q + kof * Sp - (mu + J + kontime) * S
      dSp <- kontime * S - (mu + Omega * J + kof) * Sp
      dC <- - (mu + J + kontime_c + phi + gamma) * C + eff*alphatime * A + kof * Cp
      dCp <- kontime_c * C - (mu + Omega * J + kof + phi) * Cp
      dIp <- Omega * J * Sp - (mu + tau_p) * Ip
      ...
      dIc[,1] <- (phi + J) * C - (mu + rho[1] + tau_c[1]) * Ic[,1]
      dI[,1] <- J * S - (mu + rho[1] + tau[1]) * I[,1] + beta * NN * qadj *(imp * prob_stages[1])
      for (k in 2:numstages) {
        dIc[,k] <- rho[k-1]*Ic[,k-1] - (mu+rho[k]+tau_c[k])*Ic[,k]
        dI[,k] <- rho[k-1]*I[,k-1] - (mu+rho[k]+tau[k])*I[,k] + beta * NN * qadj *(imp * prob_stages[k])
      }
      dIcp <- (Omega * J + phi) * Cp - (mu + tau_p) * Icp
    }
  ...
  for(f in failures){
    ...
    for(i in 1:len_post){
      ...
      for (t in 1:nt) {
        ...
        prevalence_cure[t] <- sum(Ic_out[t,,]) + sum(Icp_out[t,]) + sum(Cp_out[t,]) + sum(C_out[t,])
        incidence_cure[t] <- parms["phi"] * sum(C_out[t,]) + sum(J*C_out[t,]) + effectiveness_prep * sum(J*Cp_out[t,])
...
save.image(paste0("RESULTS/SCENARIOS/SCmix_eff", round(parms["eff"]*100,0),"_cu", round(ar*100,0), "_hc", hcfile,"_curestart",y_cure,".RData"))
