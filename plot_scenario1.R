...
load(paste0("RESULTS/SCENARIOS/SC1_eff", round(eff*100,0),"_cu", round(ar*100,0), "_hc", hcfile,"_curestart",y_cure,".RData"))
...
mean_inc_undiag_naive_nocure <- array(apply(incidence_undiag_naive_nocure_time, 2, mean), dim=c(nt))
mean_inc_undiag_nonnaive_nocure <- array(apply(incidence_undiag_nonnaive_nocure_time, 2, mean), dim=c(nt))

curet <- which(years_sim==y_cure)
cureidx <- (curet-1)*dt + 1

subset_art_given_nocure <- art_given_nocure_time[,cureidx:nt]
cumulative_art_given_nocure <- apply(subset_art_given_nocure, 1, sum) / dt
...
for (p in 1:len_post) {
  cumulative_art_averted_percure[,,p] <- (-cumulative_art_given[,,p] + cumulative_art_given_nocure[p]) / cumulative_newly_cured[,,p]
  cumulative_art_averted[,,p] <- (-cumulative_art_given[,,p] + cumulative_art_given_nocure[p]) / cumulative_art_given_nocure[p]
}
...
if(hcfile=="MAIN"){
  if(y_cure == 2026){
    pdf(file = ("PLOTS/postcure/Fig2.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale)
    save(cumulative_art_given_nocure, file = ("RESULTS/cumartgivennocure_post.RData"))
  }else{
    pdf(file = (paste0("PLOTS/postcure/FigS11",".pdf")), width = morew*size_plot, height = moreh*size_plot * rescale)
  }
}else{
  pdf(file = ("PLOTS/postcure/FigS9.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale)
}
...
if(hcfile == "MAIN" & y_cure == 2026){
  pdf(file = ("PLOTS/postcure/infectivity1.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale)
  ...
  p2_naive_mat <- matrix(NA,nrow=3,ncol=nt)
  p2_nonnaive_mat <- matrix(NA,nrow=3,ncol=nt)
  ...
  p1_mat_nocure <- mean_inc_art_nocure / perinc
  p2_mat_nocure <- mean_inc_undiag_naive_nocure / perinc
  p3_mat_nocure <- mean_inc_diag_nocure / perinc
  p_mat_nocure <- mean_inc_nocure / perinc
  
  panels <- c("b", "c", "d")
  titles <- c("Time to rebound: never", "Time to rebound: 6 years", "Time to rebound: 2 years")
  y0 <- 0
  y1 <- 1
  y1 <- 120
  y_start_plot <- 2017
  plot(0, xlim = c(y_start_plot, y_end ), ylim = c(y0, y1), col = "white", xlab = "", ylab = "Contribution to\nnew infections by status\n(100,000 person-years)", 
       xaxt = "n", yaxt = "n", bty = "n", cex.lab=cexlab)
  axis(1, at = seq(y_start_plot-1, y_end, 1), labels = NA, col = "gray72",pos=0)
  step <- 30
  axis(2, at = seq(y0, y1, step), labels = seq(y0, y1, step), cex.axis=cexlab, col = "gray72", las=2)
  text(x = seq(y_start_plot, y_end, 2), y = par("usr")[3] - textxspace * diff(par("usr")[3:4]),
       labels = seq(y_start_plot, y_end, 2),
       srt = 45, adj = c(0.5, 0.5), xpd = TRUE, cex = cexlab)
  abline(h=seq(y0,y1+step,step), lty=3, col="gray72")
  ...
  polygon(c(seq(years_sim[1],years_sim[length(years_sim)],by=1/dt), rev(seq(years_sim[1],years_sim[length(years_sim)],by=1/dt))), c((p1_mat_nocure+p2_mat_nocure+p3_mat_nocure), rev((p1_mat_nocure+p2_mat_nocure))), col = colors_inf[3], border = NA)
  mtext("a", side = 3, line = 0.5, adj = 0.0, cex = cexlab*rescmtext, font = 2)
  ...
  for(pl in 1:3){
    ...
    arrows(y_cure, par("usr")[3] + 0.25 * diff(par("usr")[3:4]), y_cure, 0, col="darkred", lwd=3, length=0.1)
    if(pl==3){
      legend("top", legend = c("Diagnosed", "Undiagnosed rebounded", "Undiagnosed naive", "Treated"), col = "white", pt.bg = rev(colors_inf), pch = rep(22,3), pt.cex = 4, cex = cexlab, bty = 'n')
    }
  }
  ...
  bp_caa <- rowMeans(cumulative_art_averted[g,,])
  bp_caa_lo <- apply(cumulative_art_averted[g,,], 1, quantile, prob = 0.025)
  bp_caa_hi <- apply(cumulative_art_averted[g,,], 1, quantile, prob = 0.975)
  
  pdf(file = ("PLOTS/postcure/artaverted1.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale)
  layout(matrix(c(1,1, 2,2, 3, 3,4,4), 2, 4, byrow = TRUE))
  ...
  text(x = seq(1, 3, 1), y = par("usr")[3] - textxspace * diff(par("usr")[3:4]),
       labels = c("never", "6 years", "2 years"),
       srt = 45, adj = c(0.5, 0.5), xpd = TRUE, cex = cexlab)
  abline(h=seq(y0,y1+step,step), lty=3, col="gray72")
  leg_sa <- rep("",n_sa)
  for(i in 1:n_sa){
  ...
  mtext("a", side = 3, line = 0.5, adj = 0.0, cex = cexlab*rescmtext, font = 2)
  arrows(y_cure, par("usr")[3] + 0.25 * diff(par("usr")[3:4]), y_cure, 0, col="darkred", lwd=3, length=0.1)
  mtext("Time to rebound", side=1, outer=TRUE, line=-4, at=0.82, cex=cexlab*rescyear, adj = c(0.5, 0.5), font=1)  # Below the D panel
  
  dev.off()
  
  pdf(file = ("PLOTS/postcure/artpercure1.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale)
  ...
  bp_caa_percure <- rowMeans(cumulative_art_averted_percure[g,,])
  bp_caa_percure_lo <- apply(cumulative_art_averted_percure[g,,], 1, quantile, prob = 0.025)
  bp_caa_percure_hi <- apply(cumulative_art_averted_percure[g,,], 1, quantile, prob = 0.975)
  ...
  leg_sa <- rep("",n_sa)
  for(i in 1:n_sa){
    rect(xleft=i-0.48, ybottom = 0, xright = i+0.48, ytop=bp_caa_percure[i], col = colors1[i], border = "white")
  ...
  rescyear <- 0.69*morew/2
  rescpo <- 1.5
  pdf(file = ("PLOTS/postcure/infectivity_nocure.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale)
  ...
  polygon(c(seq(years_sim[1],years_sim[length(years_sim)],by=1/dt), rev(seq(years_sim[1],years_sim[length(years_sim)],by=1/dt))), c(p1_mat[pl,], rev(rep(0,length(p_mat[pl,])))), col = colors_inf[1], border = NA)
  polygon(c(seq(years_sim[1],years_sim[length(years_sim)],by=1/dt), rev(seq(years_sim[1],years_sim[length(years_sim)],by=1/dt))), c((p1_mat[pl,]+p2_naive_mat[pl,]), rev(p1_mat[pl,])), col = colors_inf[2], border = NA)
  polygon(c(seq(years_sim[1],years_sim[length(years_sim)],by=1/dt), rev(seq(years_sim[1],years_sim[length(years_sim)],by=1/dt))), c((p1_mat[pl,]+p2_naive_mat[pl,]+p3_mat[pl,]), rev((p1_mat[pl,]+p2_naive_mat[pl,]))), col = colors_inf[3], border = NA)
  mtext("d", side = 3, line = 0.5, adj = 0.0, cex = cexlab*rescmtext, font = 2)
  arrows(y_cure, par("usr")[3] + 0.25 * diff(par("usr")[3:4]), y_cure, 0, col="darkred", lwd=3, length=0.1)
  ...
  # never
  p1_mat[1,length(p1_mat[1,])] / p_mat[1,length(p1_mat[1,])]
  p2_naive_mat[1,length(p1_mat[1,])] / p_mat[1,length(p1_mat[1,])]
  p2_nonnaive_mat[1,length(p1_mat[1,])] / p_mat[1,length(p1_mat[1,])]
  p3_mat[1,length(p1_mat[1,])] / p_mat[1,length(p1_mat[1,])]
  # 6 years
...
mean_inc_reb[g,2,length(mean_inc_new[g,1,])] / perinc
q1_inc_reb[g,2,length(mean_inc_new[g,1,])] / perinc
q2_inc_reb[g,2,length(mean_inc_new[g,1,])] / perinc
...
mean_inc_reb[g,3,length(mean_inc_new[g,1,])] / perinc
q1_inc_reb[g,3,length(mean_inc_new[g,1,])] / perinc
q2_inc_reb[g,3,length(mean_inc_new[g,1,])] / perinc

mean_inc_nocure[length(mean_inc_nocure)] / perinc
lo_inc_nocure[length(mean_inc_nocure)] / perinc
hi_inc_nocure[length(mean_inc_nocure)] / perinc

yf_idxs
yf_2024 <- yf_idxs[length(yf_idxs)] + dt*3
p1_mat[1,yf_2024] / p_mat[1,yf_2024]
p2_naive_mat[1,yf_2024] / p_mat[1,yf_2024]
p3_mat[1,yf_2024] / p_mat[1,yf_2024]
