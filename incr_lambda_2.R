...
# Define the year when probability of transmission starts increasing
y_incr <- 2022
t_incr <- which(years_sim_extend==y_incr) - 1

# Define the slope of linear increase of lambda
slopelambda <- 0.001
...
model <- function(t, x, parms) {
  with(as.list(c(parms, x)), {
    ...
    if(t<t_incr){
      ...
      lambdatime <- parms["lambda"] + (t-t_incr)*slopelambda
      J <- rep(0,numgroups)
      for(l in 1:numgroups){
        for (ll in 1:numgroups) {
          J[l] <- J[l] + M[l, ll] * (eps_p * (Ip[ll] + Icp[ll]) / Ng[ll] + eps * (A[ll] + R[ll]) / Ng[ll] + eps_d * D[ll] / Ng[ll])
    ...
    for(i in 1:len_post){
      ...
      for (t in 1:nt) {
        ...
        if(t<=dt*t_incr+1){
          ...
          lambdatime <- parms["lambda"] + slopelambda*((t-1)/dt-t_incr)
          J <- rep(0,numgroups)
          for(l in 1:numgroups){
            for (ll in 1:numgroups) {
              J[l] <- J[l] + M[l, ll] * (transmissibility_prep * (Ip_out[t,ll] + Icp_out[t,ll]) / Ng_out[t,ll] + parms["eps"] * (A_out[t,ll] + R_out[t,ll]) / Ng_out[t,ll] + transmissibility_diagnosed * D_out[t,ll] / Ng_out[t,ll])
            ...
            J[l] <- J[l] * (lambdatime * contact_rates[l])
          }
        }
...
save.image(paste0("RESULTS/SCENARIOS/lambda_increased_SC2.RData"))
