...
n_rows <- 3
cex_scale <- n_columns
pdf(file = "PLOTS/precure/FigS5_noimp.pdf", width = n_columns*size_plot, height = n_rows*size_plot * rescale2)
...
axis(2, at = seq(0,max_counts+step*bin2*sum(counts),10), label=round(seq(0,max_counts+step*bin2*sum(counts),10)/sum(counts),2), cex.axis=cex_scale, las=2, col="gray72", pos=0)
...
bins <- seq(y0,y1+step*bin1,step*bin1)
axis(1, at = bins, col="gray72", las=2, cex.axis=cex_scale, pos=0)
axis(2, at = seq(0,max_counts,5), label=round(seq(0,max_counts,5)/sum(counts),2), cex.axis=cex_scale, las=2, col="gray72", pos=0)
mtext("Frequency", side = 2, line = 8.5, cex = 2.5)
...
bins <- seq(y0,y1+step,step)
hist(vec, breaks = bins, col = "white", border = "white", xlab="", ylab="", xaxt="n", yaxt="n", bty='n', main = NULL)
hist_data <- hist(vec, breaks = bins, plot = FALSE)
counts <- hist_data$counts
max_counts <- max(counts)
norm_counts <- counts / sum(counts)
...
bins <- seq(0,y1,0.01)
axis(1, at = bins, col="gray72", las=2, cex.axis=cex_scale, pos=0)
axis(2, at = seq(0,max_counts+step*bin2*sum(counts),5), label=round(seq(0,max_counts+step*bin2*sum(counts),5)/sum(counts),2), cex.axis=cex_scale, las=2, col="gray72", pos=0)
mtext("Frequency", side = 2, line = 8.5, cex = 2.5)
...
# idx_not <- c()
# idx_not <- c(idx_not,which(names(datacor)=="tau1"))
# idx_not <- c(idx_not,which(names(datacor)=="tau2"))
# idx_not <- c(idx_not,which(names(datacor)=="NN"))
# datacor <- datacor[-idx_not,-idx_not]
...
ggsave("PLOTS/precure/FigS6_noimp.pdf", plot = heatmap, width = 1.25*size_plot, height = size_plot)
...
rescpo <- 1.5
pdf(file = ("PLOTS/precure/Fig1_noimp.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale *addresc)
...
pdf(file = ("PLOTS/precure/FigS1_noimp.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale * addresc)
...
lines(seq(years_sim[1],years_sim[length(years_sim)],by=1/dt), mean_impdiag - mean_impdiag, col = adjustcolor(colcurr, alpha = 1), lwd = moreh*morew*resclinew)
...
step <- 0.2
yy0 <- floor(y0 / step) * step
y1 <- max(c((max(ci_art2[1,])),max(treat_currently / currently_infected_est)))
y1 <- 1
...
mtext("Year", side=1, outer=TRUE, line=1., at=0.565, cex=cexlab*rescyear, adj = c(0.5, 0.5))  # Below the C panel
legend("bottomright", legend = c("Data", "Model"), col = c(colrom1, colcurr), pch = c(19, NA), lty = c(NA, 1), lwd=c(.75,moreh), cex = cexlab*rescleg, bty = "n")
dev.off()
