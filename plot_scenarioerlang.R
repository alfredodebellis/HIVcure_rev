...
load(paste0("RESULTS/SCENARIOS/SC1_erlangE3_eff", round(eff*100,0),"_cu", round(ar*100,0), "_hc", hcfile,"_curestart",y_cure,".RData"))
...
if(hcfile=="MAIN"){
  if(y_cure == 2026){
    pdf(file = ("PLOTS/postcure/Fig2_erlang.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale)
  }else{
    pdf(file = (paste0("PLOTS/postcure/FigS11","_erlang.pdf")), width = morew*size_plot, height = moreh*size_plot * rescale)
  }
}else{
  pdf(file = ("PLOTS/postcure/FigS9_erlang.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale)
}
...
leg_sa <- rep("",n_sa)
for(i in 1:n_sa){
  lines(seq(years_sim[first_year_fit_idx]+1,years_sim[length(years_sim)],by=1/dt), mean_inc_reb[g,i,(dt*first_year_fit_idx+1):(dt*length(years_sim)-dt+1)] / perinc / E, col = colors1[i], lwd = moreh*morew*resclinew)
  polygon(c(seq(years_sim[first_year_fit_idx]+1,years_sim[length(years_sim)],by=1/dt), rev(seq(years_sim[first_year_fit_idx]+1,years_sim[length(years_sim)],by=1/dt))), c(q1_inc_reb[g,i,(dt*first_year_fit_idx+1):(dt*length(years_sim)-dt+1)] / perinc / E, rev(q2_inc_reb[g,i,(dt*first_year_fit_idx+1):(dt*length(years_sim)-dt+1)]) / perinc / E), col = adjustcolor(colors1[i], alpha = 0.1), border = NA)
  leg_sa[i] <- 1 / failures[i]
}
