...
n_rows <- 3
cex_scale <- n_columns
pdf(file = "PLOTS/precure/FigS5_fixedimp.pdf", width = n_columns*size_plot, height = n_rows*size_plot * rescale2)
...
bins <- seq(y0,y1+step*bin1,step*bin1)
bins <- seq(0,y1,0.01)
axis(1, at = (mean(accp_list$imp)), labels = round(mean(accp_list$imp),4), col="gray72", las=2, cex.axis=cex_scale, pos=0)
...
ggsave("PLOTS/precure/FigS6_fixedimp.pdf", plot = heatmap, width = 1.25*size_plot, height = size_plot)
...
rescpo <- 1.5
pdf(file = ("PLOTS/precure/Fig1_fixedimp.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale *addresc)
...
pdf(file = ("PLOTS/precure/FigS1_fixedimp.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale * addresc)
