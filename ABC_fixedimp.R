...
m_imp <- as.numeric(lm_fit_imp$coefficients)
...
# Fix the importation undiagnosed/diagnosed ratio
fr <- undiag_2022 / (-undiag_2022 + prev_2022)
fniu <- fr * newimp_2022
fniu_resc <- fniu / (birth_rate * initial_population)
...
parms_fit_ranges <- matrix(c(0.001, 0.1, #lambda
                             0, 1, #omega
                             0.00001, 0.075, #eps
                             0, currently_infected_est[1] * 30 / 100, #ur
                             fniu_resc, fniu_resc, #imp
                             0.001, 1, #qadj1
                             0.001, 1, #qadj2
                             0.001, 1, #qadj3
                             0.001, 1), #qadj4
...
save.image(paste0("RESULTS/fit_threshold",round(100*threshold_fit,0),"_fixedimp.RData"))
