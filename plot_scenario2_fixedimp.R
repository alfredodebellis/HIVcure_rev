...
y_cure <- 2026
load(paste0("RESULTS/SCENARIOS/SC2_eff", round(eff*100,0), "_hc", hcfile,"_curestart",y_cure,"_fixedimp.RData"))
...
load("RESULTS/cumartgivennocure_post_fixedimp.RData")
...
if(hcfile=="MAIN"){
  if(y_cure == 2026){
    pdf(file = ("PLOTS/postcure/Fig4_fixedimp.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale)
  }else{
    pdf(file = (paste0("PLOTS/postcure/FigS12","_fixedimp.pdf")), width = morew*size_plot, height = moreh*size_plot * rescale)
  }
}else{
  pdf(file = ("PLOTS/postcure/FigS10_fixedimp.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale)
}
