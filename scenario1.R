...
# for (g in 2:2) {
  for(f in failures){
    ...
    for(i in 1:len_post){
      ...
      incidence_undiag_naive_after_new_time_pool[g,which(failures==f),i,] <- incidence_undiag_naive
      incidence_undiag_nonnaive_after_new_time_pool[g,which(failures==f),i,] <- incidence_undiag_nonnaive
...
#Simulate scenario with no cure with the same samples
parms["alpha"] <- 0

incidence_nocure_time <- array(0,dim=c(len_post,nt))
incidence_art_nocure_time <- array(0,dim=c(len_post,nt))
incidence_diag_nocure_time <- array(0,dim=c(len_post,nt))
incidence_undiag_naive_nocure_time <- array(0,dim=c(len_post,nt))
incidence_undiag_nonnaive_nocure_time <- array(0,dim=c(len_post,nt))
prevalence_nocure_time <- array(0,dim=c(len_post,nt))
art_given_nocure_time <- array(0,dim=c(len_post,nt))
...
for(i in 1:len_post){
  ...
  incidence_nocure_time[i,] <- incidence
  incidence_art_nocure_time[i,] <- incidence_art
  incidence_diag_nocure_time[i,] <- incidence_diag
  incidence_undiag_naive_nocure_time[i,] <- incidence_undiag_naive
  incidence_undiag_nonnaive_nocure_time[i,] <- incidence_undiag_nonnaive
  prevalence_nocure_time[i,] <- prevalence
  art_given_nocure_time[i,] <- art_given
}

mean_inc_nocure <- array(apply(incidence_nocure_time, 2, mean), dim=c(nt))
cilow_inc_nocure <- array(apply(incidence_nocure_time, 2, quantile, prob=0.025), dim=c(nt))
cihigh_inc_nocure <- array(apply(incidence_nocure_time, 2, quantile, prob=0.025), dim=c(nt))
mean_prev_nocure <- array(apply(prevalence_nocure_time, 2, mean), dim=c(nt))
cilow_prev_nocure <- array(apply(prevalence_nocure_time, 2, quantile, prob=0.025), dim=c(nt))
cihigh_prev_nocure <- array(apply(prevalence_nocure_time, 2, quantile, prob=0.025), dim=c(nt))
...
mean_inc_undiag_nonaive_nocure <- array(apply(incidence_undiag_nonnaive_nocure_time, 2, mean), dim=c(nt))

mean_art_given_nocure <- array(apply(art_given_nocure_time, 2, mean), dim=c(nt))

# Reset cure uptake as in the beginning
parms["alpha"] <- -log(1-ar)

save.image(paste0("RESULTS/SCENARIOS/SC1_eff", round(parms["eff"]*100,0),"_cu", round(ar*100,0), "_hc", hcfile,"_curestart",y_cure,".RData"))
