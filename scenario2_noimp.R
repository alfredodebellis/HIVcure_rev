...
load("RESULTS/fit_threshold15_noimp.RData")
...
model <- function(t, x, parms) {
  with(as.list(c(parms, x)), {
    ...
    if(t<times[dt*(prep_fit_idxs[1]-1)+1]){
      ...
      dI[,1] <- J * S - (mu + rho[1] + tau[1]) * I[,1] 
      for (k in 2:numstages) {
        dI[,k] <- rho[k-1]*I[,k-1] - (mu+rho[k]+tau[k])*I[,k]
      }
...
for (g in 2:2) {
  for(f in alphas){
    ...
    for(i in 1:len_post){
      ...
      for (t in 1:nt) {
        ...
        for(l in 1:numgroups){
          for (ll in 1:numgroups) {
            ...
            J_undiag[l] <- J_undiag[l] +  M[l, ll] * transmissibility_prep * (Ip_out[t,ll] + Icp_out[t,ll]) / Ng_out[t,ll]
      ...
      incidence_undiag_after_new_time_pool[g,which(alphas==f),i,] <- incidence_undiag
...
save.image(paste0("RESULTS/SCENARIOS/SC2_eff", round(parms["eff"]*100,0), "_hc", hcfile,"_curestart",y_cure,"_noimp.RData"))
