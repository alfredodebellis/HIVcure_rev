...
for(i in 1:len_post){
  ...
  for (t in 1:nt) {
    ...
    if(t<=dt*t_incr+1){
      ...
      for(l in 1:numgroups){
        for (ll in 1:numgroups) {
          J[l] <- J[l] + M[l, ll] * (transmissibility_prep * Ip_out[t,ll] / Ng_out[t,ll] + parms["eps"] * (A_out[t,ll] + D_out[t,ll] + R_out[t,ll]) / Ng_out[t,ll])
          for (k in 1:numstages) {
            J[l] <- J[l] + M[l, ll] * (infectivity[k] * I_out[t,ll,k] + hc[k] * Ic_out[t,ll,k]) / Ng_out[t,ll]
          }
        }
...
save.image(paste0("RESULTS/SCENARIOS/lambda_increased_SC1.RData"))
