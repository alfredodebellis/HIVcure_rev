...
load(paste0("RESULTS/SCENARIOS/SC1_eff", round(eff*100,0),"_cu", round(ar*100,0), "_hc", hcfile,"_curestart",y_cure,"_noimp.RData"))
...
save(cumulative_art_given_nocure, file = ("RESULTS/cumartgivennocure_post_noimp.RData"))
...
if(hcfile=="MAIN"){
  if(y_cure == 2026){
    pdf(file = ("PLOTS/postcure/Fig2_noimp.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale)
  }else{
    pdf(file = (paste0("PLOTS/postcure/FigS11","_noimp.pdf")), width = morew*size_plot, height = moreh*size_plot * rescale)
  }
}else{
  pdf(file = ("PLOTS/postcure/FigS9_noimp.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale)
}
...
rescyear <- 0.69*morew/2
rescpo <- 1.5
pdf(file = ("PLOTS/postcure/infectivity_nocure_noimp.pdf"), width = morew*size_plot, height = moreh*size_plot * rescale)
