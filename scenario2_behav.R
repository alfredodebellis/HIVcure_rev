...
contact_rates_sc2 <- read_excel("DATA/SEXUAL_BEHAVIOR/meanpartnerchangerates_total_2.xlsx")
contact_rates_sc2 <- as.numeric(names(contact_rates_sc2))
contact_rates_sc2 <- unlist(contact_rates_sc2)[1:(length(contact_rates_sc2)-1)]
contact_rates_sc2 <- contact_rates_sc2 * 1.5

# Decide what kind of behavioral changes has to be implemented
half <- 1 # half increase behav
# half <- 0 # regular increase behav
if(half == 1){
  contact_rates_sc2 <- (contact_rates_sc2 + contact_rates) / 2
  halffile <- "HALF"
}else{
  halffile <- ""
}
...
celi <- contact_rates_sc2
parms <- c(parms, celi)
names(parms)[(length(parms)-4+1):length(parms)] <- sapply(1:numstages, function(l) paste0("celi", l))
alpha <- 1
parms <- c(parms, alpha)
...
# Model function for scenario 2 with behavioral changes
model <- function(t, x, parms) {
  with(as.list(c(parms, x)), {
    ...
    celi <- sapply(1:numgroups, function(l) parms[paste0("celi", l)])
      ...
      for(l in 1:numgroups){
        for (ll in 1:numgroups) {
          ...
          for (k in 1:numstages) {
            J[l] <- J[l] + M[l, ll] * h[k] * (I[ll, k] + Ic[ll, k]) / Ng[ll]
          }
        }
        J[l] <- J[l] * (lambda * c[l])
      }
    }else{
      celitime <- rep(0,numgroups)
      for(l in 1:numgroups){
        celitime[l] <- celi[l] * 1 / (1 + (celi[l] - c[l]) / c[l] * exp(-(r/2)*(t-t_cure)) )
      }
      M <- matrix(0,numgroups,numgroups)
      denM <- sum(celitime*Ng)
      for(l in 1:numgroups){
        for (ll in 1:numgroups) {
          M[l,ll] <- omega * celitime[ll]*Ng[ll] / denM
          if(l==ll){
            M[l,ll] <- M[l,ll] + (1 - omega)
          }
        }
    ...
    for(i in 1:len_post){
      ...
      for (t in 1:nt) {
        if(t<=dt*t_cure+1){
          ...
          celitimeext <- rep(0,numgroups)
          for(l in 1:numgroups){
            celitimeext[l] <- contact_rates_sc2[l] * 1 / (1 + (contact_rates_sc2[l] - contact_rates[l]) / contact_rates[l] * exp(-(parms["r"]/(2*dt))*(t-dt*t_cure-1)) )
          }
          M <- matrix(0,numgroups,numgroups)
          denM <- sum(celitimeext*Ng_out[t,])
          for(l in 1:numgroups){
            for (ll in 1:numgroups) {
              M[l,ll] <- parms["omega"] * celitimeext[ll]*Ng_out[t,ll] / denM
              if(l==ll){
                M[l,l] <- M[l,l] + (1 - parms["omega"])
              }
            }
          ...
          for(l in 1:numgroups){
            ...
            J[l] <- J[l] * (parms["lambda"] * celitimeext[l])
          }
        }
...
save.image(paste0("RESULTS/SCENARIOS/SC2_eff", round(parms["eff"]*100,0),"_curestart",y_cure,"_behav",halffile, ".RData"))
